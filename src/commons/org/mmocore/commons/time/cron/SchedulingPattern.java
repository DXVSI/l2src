	...
	public boolean match(TimeZone timezone, long millis)
	{
		...
		for (int i = 0; i < matcherSize; i++)
		{
			...
			boolean eval = minuteMatcher.match(minute)
			        ...
			        && (dayOfMonthMatcher instanceof DayOfMonthValueMatcher ? ((DayOfMonthValueMatcher) dayOfMonthMatcher).match(dayOfMonth, month, gc.isLeapYear(year)) : dayOfMonthMatcher
			                .match(dayOfMonth)) && monthMatcher.match(month) && dayOfWeekMatcher.match(dayOfWeek);
			if (eval)
				return true;
		}
		return false;
			...
			loop: for (;;)
			{
				...
				for (int month = gc.get(Calendar.MONTH) + 1; month <= MONTH_MAX_VALUE; month++)
				{
					if (monthMatcher.match(month))
					{
						...
						{
							if (dayOfMonthMatcher instanceof DayOfMonthValueMatcher ? ((DayOfMonthValueMatcher) dayOfMonthMatcher).match(dayOfMonth, month, isLeapYear) : dayOfMonthMatcher
							        .match(dayOfMonth))
							{
