...
public class Scripts {
	...
	private void load() {
		...
		/*
		 * File f = new File("scripts.jar"); if (f.exists()) { JarInputStream stream = null; try { stream = new JarInputStream(new
		 * FileInputStream(f)); JarEntry entry = null; while ((entry = stream.getNextJarEntry()) != null) { // Вложенные класс if
		 * (entry.getName().contains(ClassUtils.INNER_CLASS_SEPARATOR) || !entry.getName().endsWith(".class")) continue;
		 * 
		 * String name = entry.getName().replace(".class", "").replace("/", ".");
		 * 
		 * Class<?> clazz = Class.forName(name); if (Modifier.isAbstract(clazz.getModifiers())) continue; classes.add(clazz); } result = true; } catch
		 * (Exception e) { _log.error("Fail to load scripts.jar!", e); classes.clear(); } finally { IOUtils.closeQuietly(stream); } }
		 * 
		 * if (!result)
		 */
	...
	private boolean load(List<Class<?>> classes, String target, boolean reload) {
		...
		// При проверке scripts.jar, если в data/scripts/* есть такой же java файл, то этот файл берется из датапака, а не из jar
		// (Метод обновления файлов)
		if (f.exists()) {
			_log.info("Loading scripts from jar");
			...
			try {
				...
				while ((entry = stream.getNextJarEntry()) != null) {
					...
					// Если есть обновленный java класс - то отправляем на перекомпиляцию
					if (recompile)
						continue;

					// Если обновленного класса нет - загружаем
		...
		} else */if (!reload) {   // Проверка ранее загруженных в память классов, помимо scripts.jar
			// При проверке других способов загрузки - из файлов, находящихся в data/scripts формируется список,
			// из которого удаляются классы, ранее загруженные в память
			// (Метод исключения)
