				...
				if (mult > 0)
				{
					...
					for (int i = 0; i < Config.MMO_TOP_REWARD.length; i += 2)
						if (Config.MMO_TOP_REWARD[i] == -100) // PC Bang
						{
							...
							Log.add(player.getName() + " | " + player.getObjectId() + " | MMOTop reward item ID | " + Config.MMO_TOP_REWARD[i] + " | MMOTop reward count | "
							        + Config.MMO_TOP_REWARD[i + 1] * mult + " |", "mmotop");
						}
						                                           ...
						                                           // reputation
						{
							...
							else
							{
								...
								Log.add(player.getName() + " | " + player.getObjectId() + " | MMOTop reward item ID | " + Config.MMO_TOP_REWARD_NO_CLAN[i] + " | MMOTop reward count | "
								        + Config.MMO_TOP_REWARD_NO_CLAN[i + 1] * mult + " |", "mmotop");
							}
						}
