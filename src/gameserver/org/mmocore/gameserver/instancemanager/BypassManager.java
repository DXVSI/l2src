...
public class BypassManager
{
	...
	public static BypassType getBypassType(String bypass)
	{
		switch (bypass.charAt(0))
		{
			...
			default:
				if (Strings.matches(bypass, "^(_bbshome|_bbslink|_bbsreglist|_bbsgetfav|_bbsloc|_bbsregion|_clbbsclan|_bbsclan|_bbsmemo|_maillist|_friendlist|_bbsaddfav).*", Pattern.DOTALL))
	...
	public static DecodedBypass decode(String bypass, GArray<String> bypassStorage, boolean bbs, Player player)
	{
		synchronized (bypassStorage)
		{
			...
			if (bp == null)
			{
				Log.add("Can't decode bypass (bypass not exists): " + (bbs ? "[bbs] " : "") + bypass + " / Player: " + player.getName() + " / Npc: "
