				...
				if (mult > 0)
				{
					...
					for (int i = 0; i < Config.L2_TOP_REWARD.length; i += 2)
						if (Config.L2_TOP_REWARD[i] == -100) // PC Bang
						{
							...
							Log.add(player.getName() + " | " + player.getObjectId() + " | L2Top reward item ID | " + Config.L2_TOP_REWARD[i] + " | L2Top reward count | " + Config.L2_TOP_REWARD[i + 1]
							        * mult + " |", "l2top");
						}
						                                          ...
						                                          // reputation
						{
							...
							else
							{
								...
								Log.add(player.getName() + " | " + player.getObjectId() + " | L2Top reward item ID | " + Config.L2_TOP_REWARD_NO_CLAN[i] + " | L2Top reward count | "
								        + Config.L2_TOP_REWARD_NO_CLAN[i + 1] * mult + " |", "l2top");
							}
						}
