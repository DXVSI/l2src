		...
		for (String arrayedData : arrayedDatas)
		{
			...
			if (formedStr.length > 1)
			{
				...
				if (dc.time + Config.SMS_PAYMENT_SAVE_DAYS * 86400 > System.currentTimeMillis() / 1000)
					checkAndSave(dc.id, dc.service_id, dc.status, dc.time, dc.curr_id, dc.sum, dc.profit, dc.email, dc.client_id, Integer.parseInt(outputCollection.get("wNumber")),
					        outputCollection.get("wPhone"), outputCollection.get("wText"), Float.parseFloat(outputCollection.get("wCost")), Float.parseFloat(outputCollection.get("wProfit")),
					        outputCollection.get("wCountry"));
			}
		}
	}

	public void checkAndSave(int id, int service_id, int status, long time, int curr_id, float sum, float profit, String email, int client_id, int wNumber, String wPhone, String wText, float wCost,
	        float wProfit, String wCountry)
	{
		int objId = 0;
				...
				if (profit > 0)
				{
					...
					for (int i = 0; i < Config.SMS_PAYMENT_REWARD.length; i += 2)
						if (Config.SMS_PAYMENT_REWARD[i] == -100) // PC Bang
						{
							...
							Log.add(player.getName() + " | " + player.getObjectId() + " | SMS reward item ID | " + Config.SMS_PAYMENT_REWARD[i] + " | SMS reward count | "
							        + (int) (Config.SMS_PAYMENT_REWARD[i + 1] * profit) + " |", "sms");
						}
						                                               ...
						                                               // reputation
						{
							...
							else
							{
								...
								Log.add(player.getName() + " | " + player.getObjectId() + " | SMS reward item ID | " + Config.SMS_PAYMENT_REWARD_NO_CLAN[i] + " | SMS reward count | "
								        + (int) (Config.SMS_PAYMENT_REWARD_NO_CLAN[i + 1] * profit) + " |", "sms");
							}
						}
