...
public final class SubClassInfo
{
	...
	public static Set<ClassId> getAvailableSubClasses(Player player, Race npcRace, ClassType npcTeachType, boolean isNew)
	{
		...
		for (ClassId availSub : availSubs)
		{
			...
			{
				if (((currClass == ClassId.M_SOUL_HOUND) || (currClass == ClassId.F_SOUL_HOUND) || (currClass == ClassId.F_SOUL_BREAKER) || (currClass == ClassId.M_SOUL_BREAKER))
				        && ((availSub == ClassId.F_SOUL_BREAKER) || (availSub == ClassId.M_SOUL_BREAKER)))
				{
					availSubs.remove(availSub);
				}
