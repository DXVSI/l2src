...
public class CastleSiegeEvent extends SiegeEvent<Castle, SiegeClanObject>
{
	...
	public void processStep(Clan newOwnerClan)
	{
		...
		{
			// Если атакуется замок, принадлежащий NPC, и только 1 атакующий -
			// закончить осаду
			...
			// Если атакуется замок, принадлежащий NPC, и все атакующие в одном
			// альянсе - закончить осаду
		...
		// ток при первом захвате обнуляем мерчант гвардов и убираем апгрейд
		// дверей
		if (!_firstStep)
		{
			_firstStep = true;
	...
	public void stopEvent(boolean step)
	{
		...
		if (ownerClan != null)
		{
			...
			else
			{
				...
				if (_oldOwner != null)
					_oldOwner.broadcastToOnlineMembers(new SystemMessage2(
					        SystemMsg.YOUR_CLAN_HAS_FAILED_TO_DEFEND_THE_CASTLE_S1_POINTS_HAVE_BEEN_DEDUCTED_FROM_YOU_CLAN_REPUTATION_SCORE_AND_ADDED_TO_YOUR_OPPONENTS).addInteger(-_oldOwner
					        .incReputation(-3000, false, toString())));
	...
	public void reCalcNextTime(boolean onInit)
	{
		...
		if (onInit)
		{
			...
			{
				if (currentTimeMillis - ownSiegeDate.getTimeInMillis() > DAY_IN_MILISECONDS) // прошол
				                                                                             // день
				                                                                             // после
				                                                                             // осады
					setNextSiegeTime();
	...
	/**
	 * Ставит осадное время вручну, вызывается с пакета {@link org.mmocore.gameserver.network.clientpackets.RequestSetCastleSiegeTime}
	 * 
	 * @param id
	 */
