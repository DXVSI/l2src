...
import org.mmocore.gameserver.model.entity.residence.ResidenceType;
...
public class Clan implements Iterable<UnitMember>
{
	...
	public void store()
	{
		...
		try
		{
			con = DatabaseFactory.getInstance().getConnection();
			statement = con
			        .prepareStatement("INSERT INTO clan_data (clan_id,clan_level,hasCastle,hasFortress,hasHideout,ally_id,expelled_member,leaved_ally,dissolved_ally,airship) values (?,?,?,?,?,?,?,?,?,?)");
	...
	public static Clan restore(int clanId)
	{
		...
		try
		{
			...
			statement1 = con1
			        .prepareStatement("SELECT clan_level,hasCastle,hasFortress,hasHideout,ally_id,reputation_score,expelled_member,leaved_ally,dissolved_ally,warehouse,airship FROM clan_data where clan_id=?");
