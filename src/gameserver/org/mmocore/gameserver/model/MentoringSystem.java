	...
	public void remove(String name, boolean isMentor, boolean notify) {
		...
		if (objectId > 0 && notify) {
			...
			if (otherSideMentee != null) {
				otherSideMentee.sendPacket(new SystemMessage2(
						SystemMsg.THE_MENTORING_RELATIONSHIP_WITH_S1_HAS_BEEN_CANCELED).addString(isMentor ? name
						: mentor.getName()));
				MentorUtil.removeEffectsFromPlayer(otherSideMentee);
			}
			mentor.sendPacket(new SystemMessage2(SystemMsg.THE_MENTORING_RELATIONSHIP_WITH_S1_HAS_BEEN_CANCELED)
					.addString(isMentor ? name : mentor.getName()));
		}
	}
	...
	public void notify(boolean online) {
		for (MenteeInfo mentee : menteeInfo) {
			...
			if (menteePlayer != null) {
				...
				if (online) {
					menteePlayer.sendPacket(new SystemMessage2(
							mentee.isMentor() ? SystemMsg.YOU_MENTEE_S1_HAS_CONNECTED
									: SystemMsg.YOU_MENTOR_S1_HAS_CONNECTED).addString(mentor.getName()));
				} else {
					menteePlayer.sendPacket(new SystemMessage2(
							mentee.isMentor() ? SystemMsg.YOU_MENTEE_S1_HAS_DISCONNECTED
									: SystemMsg.YOU_MENTOR_S1_HAS_DISCONNECTED).addString(mentor.getName()));
				}
