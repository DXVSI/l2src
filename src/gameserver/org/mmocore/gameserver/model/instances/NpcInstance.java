...
import org.mmocore.gameserver.network.serverpackets.AcquireSkillDone;
import org.mmocore.gameserver.network.serverpackets.AcquireSkillList;
import org.mmocore.gameserver.network.serverpackets.AutoAttackStart;
import org.mmocore.gameserver.network.serverpackets.ExChangeNpcState;
import org.mmocore.gameserver.network.serverpackets.ExShowBaseAttributeCancelWindow;
import org.mmocore.gameserver.network.serverpackets.ExShowCommission;
import org.mmocore.gameserver.network.serverpackets.ExShowUsmVideo;
import org.mmocore.gameserver.network.serverpackets.ExShowVariationCancelWindow;
import org.mmocore.gameserver.network.serverpackets.ExShowVariationMakeWindow;
			...
			for (TeleportLocation tl : list)
				if (tl.getItem().getItemId() == ItemTemplate.ITEM_ID_ADENA)
				{
					...
					sb.append("[scripts_Util:QuestGatekeeper ").append(tl.getX()).append(" ").append(tl.getY()).append(" ").append(tl.getZ()).append(" ").append(tl.getPrice()).append(" ")
					        .append(tl.getItem().getItemId()).append(" @811;F;").append("|").append(HtmlUtils.htmlNpcString(tl.getName())).append(" - ").append(tl.getPrice()).append(" ")
	...
	public void showSkillList(Player player)
	{
		...
		if (getTemplate().getTeachInfo().isEmpty())
		{
			...
			if (player.getVar("lang@").equalsIgnoreCase("en"))
				...
				sb.append("Я не погу обучить тебя. Для твоего класса пой список пуст.<br> Свяжись с адпиноп для фикса этого. <br>NpcId:" + npcId + ", твой classId:" + player.getClassId().getId()
				        + "<br>");
			sb.append("</body></html>");
			html.setHtml(sb.toString());
			player.sendPacket(html);
		...
		if (!(getTemplate().canTeach(classId) || getTemplate().canTeach(classId.getParent(player.getSex()))))
		{
			...
			else
			{
				...
				StringBuilder sb = new StringBuilder();
				sb.append("<html><head><body>");
				sb.append(new CustomMessage("org.mmocore.gameserver.model.instances.L2NpcInstance.WrongTeacherClass", player));
				sb.append("</body></html>");
				html.setHtml(sb.toString());
				player.sendPacket(html);
			}
