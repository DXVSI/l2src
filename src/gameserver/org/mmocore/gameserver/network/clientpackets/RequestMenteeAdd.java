...
import org.mmocore.gameserver.network.serverpackets.ExMentorAdd;
...
public class RequestMenteeAdd extends L2GameClientPacket {
	...
	protected void runImpl() {
		...
		if (newMentee.getLevel() > 85) {
			activeChar.sendPacket(new SystemMessage2(SystemMsg.S1_IS_ABOVE_LEVEL_86_AND_CANNOT_BECOME_A_MENTEE)
					.addName(newMentee));
			return;
		}
		...
		if (newMentee.getInventory().getItemByItemId(MENTEE_CERTIFICATE) == null) {
			activeChar.sendPacket(new SystemMessage2(SystemMsg.S1_DOES_NOT_HAVE_THE_ITEM_NEDEED_TO_BECOME_A_MENTEE)
					.addName(newMentee));
			return;
		}
		...
		if (mentorPenalty > System.currentTimeMillis()) {
			...
			activeChar.sendPacket(new SystemMessage2(
					SystemMsg.YOU_CAN_BOND_WITH_A_NEW_MENTEE_IN_S1_DAYS_S2_HOUR_S3_MINUTE).addInteger(numDays)
					.addInteger(numHours).addInteger(numMins));
			return;
		}
