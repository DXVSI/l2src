...
public class RequestActionUse extends L2GameClientPacket
{
	...
	public static enum Action
	{
		...
		ACTION1106(1106, 2, 11278, 0), // Bear Claw
		ACTION1107(1107, 2, 11279, 0), // Bear Tumbling
		ACTION1108(1108, 2, 11280, 0), // Cougar Bite
		ACTION1109(1109, 2, 11281, 0), // Cougar Pounce
		ACTION1110(1110, 2, 11282, 0), // Reaper Touch
		ACTION1111(1111, 2, 11283, 0), // Reaper Power
	...
	protected void runImpl()
	{
		...
		switch (action.type)
		{
			case 0: // Действия с игроками:
			{
				...
				switch (action.id)
				{
					case 0: // Сесть/встать
						...
						{
							if (target != null && target instanceof StaticObjectInstance && ((StaticObjectInstance) target).getType() == 1
							        && activeChar.getDistance3D(target) <= Creature.INTERACTION_DISTANCE)
			...
			case 2: // Скилы петов
			{
				...
				if (action.id == 1000 && target != null && !target.isDoor()) // Siege
				                                                             // Golem
				                                                             // -
				                                                             // Siege
				                                                             // Hammer
			...
			{
				if (activeChar.isOutOfControl() || activeChar.getTransformation() != 0 || activeChar.isActionsDisabled() || activeChar.isSitting()
			...
			case 4: // Парные социальные действия
			{
				...
				if (!activeChar.isInRange(pcTarget, 300) || activeChar.isInRange(pcTarget, 25) || activeChar.getTargetId() == activeChar.getObjectId()
				        || !GeoEngine.canSeeTarget(activeChar, pcTarget, false))
				{
