...
import org.mmocore.gameserver.network.serverpackets.PrivateStoreManageListBuy;
	...
	protected void runImpl()
	{
		...
		try
		{
			loop: for (int i = 0; i < _count; i++)
			{
				...
				{
					buyer.sendMessage(new CustomMessage("org.mmocore.gameserver.clientpackets.SetPrivateStoreBuyList.TooLowPrice", buyer).addItemName(item).addNumber(item.getReferencePrice() / 2));
					continue;
				}
