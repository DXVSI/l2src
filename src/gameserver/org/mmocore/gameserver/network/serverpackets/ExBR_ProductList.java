...
public class ExBR_ProductList extends L2GameServerPacket
{
	...
	protected void writeImpl()
	{
		...
		for (ProductItem template : items)
		{
			...
			writeH(template.getCategory()); // category 1 - enchant 2 - supplies
			                                // 3 - decoration 4 - package 5 -
			                                // other
			writeD(template.getPoints()); // points
			writeD(template.getTabId()); // show tab 2-th group - 1 показывает
			                             // окошко про итем
			writeD((int) (template.getStartTimeSale() / 1000)); // start sale
			                                                    // unix date in
			                                                    // seconds
			writeD((int) (template.getEndTimeSale() / 1000)); // end sale unix
			                                                  // date in seconds
