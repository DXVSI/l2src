...
public class Formulas
{
	...
	public static boolean calcSkillSuccess(Env env, EffectTemplate et, int spiritshot)
	{
		...
		if (Config.ALT_DEBUG_ENABLED)
		{
			...
			final boolean debugPvP = Config.ALT_DEBUG_PVP_ENABLED && (debugCaster && debugTarget)
			        && (!Config.ALT_DEBUG_PVP_DUEL_ONLY || (caster.getPlayer().isInDuel() && target.getPlayer().isInDuel()));
	...
	public static void calcSkillMastery(Skill skill, Creature activeChar)
	{
		...
		if (activeChar.getSkillLevel(331) > 0 && activeChar.calcStat(Stats.SKILL_MASTERY, activeChar.getINT(), null, skill) >= Rnd.get(1000)
		        || (activeChar.getSkillLevel(330) > 0 || activeChar.getSkillLevel(10501) > 0) && activeChar.calcStat(Stats.SKILL_MASTERY, activeChar.getSTR(), null, skill) >= Rnd.get(1000))
		{
