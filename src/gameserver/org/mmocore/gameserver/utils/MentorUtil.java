...
public class MentorUtil {
	...
	public static int[] effectsForMentee = { 9233, 9227, 9228, 9229, 9230, 9231, 9232 }; // Buffs and skill SP+EXP(9233)
	public static int skillForMentee = 9379; // Summon mentor to Mentee
	public static int[] skillsForMentor = { 9376, 9377, 9378 }; // Active skills for mentor
	public static int effectForMentor = 9256; // Skill SP+EXP 10% for mentor
	public static int[] effectsRemove = { 9233, 9227, 9228, 9229, 9230, 9231, 9232, 9256 }; // Removed effects from all
																							// characters after end of
																							// mentoring
	public static int[] skillRemove = { 9376, 9377, 9378, 9379 };
	...
	public static void applyMentoringConditions(Player activeChar) {
		if (activeChar.isMentor()) {
			...
			if (World.getPlayer(activeChar.getMentorSystem().getMentor()) != null) {
				addEffectToPlayer(SkillTable.getInstance().getInfo(effectForMentor, 1),
						World.getPlayer(activeChar.getMentorSystem().getMentor()));
			}
		}
	}
	...
	public static void setTimePenalty(int mentorId, long timeTo, long expirationTime) {
		...
		if (mentor != null && mentor.isOnline()) {
			...
			mysql.set(
					"REPLACE INTO character_variables (obj_id, type, name, value, expire_time) VALUES (?,'user-var','mentorPenalty',?,?)",
					mentorId, timeTo, expirationTime);
		}
	}
