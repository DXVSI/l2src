...
public class AdminSkill implements IAdminCommandHandler
{
	...
	private void removeSkillsPage(Player activeChar)
	{
		...
		for (Skill element : skills)
			replyMSG.append("<tr><td width=80><a action=\"bypass -h admin_remove_skill " + element.getId() + "\">" + element.getName() + "</a></td><td width=60>" + element.getLevel()
			        + "</td><td width=40>" + element.getId() + "</td></tr>");
	...
	private void showEffects(Player activeChar)
	{
		...
		if (list != null && !list.isEmpty())
			for (Effect e : list)
				replyMSG.append(e.getSkill().getName()).append(" ").append(e.getSkill().getLevel()).append(" - ").append(e.getSkill().isToggle() ? "Infinity" : (e.getTimeLeft() + " seconds"))
				        .append("<br1>");
