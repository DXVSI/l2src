...
import org.mmocore.gameserver.model.actor.instances.player.SubClassInfo;
...
public class AdminEditChar implements IAdminCommandHandler
{
	...
	public boolean useAdminCommand(Enum comm, String[] wordList, String fullString, Player activeChar)
	{
		...
		else if (fullString.startsWith("admin_setsubclass"))
		{
			...
			if (st.countTokens() > 1)
			{
				...
				{
					activeChar.sendMessage(new CustomMessage("org.mmocore.gameserver.model.instances.L2VillageMasterInstance.SubclassCouldNotBeAdded", activeChar));
					return false;
				}
	...
	private void listCharacters(Player activeChar, int page)
	{
		...
		for (int i = CharactersStart; i < CharactersEnd; i++)
		{
			...
			replyMSG.append("<tr><td width=80>" + "<a action=\"bypass -h admin_character_list " + p.getName() + "\">" + p.getName() + "</a></td><td width=110>"
			        + HtmlUtils.htmlClassName(p.getClassId().getId()) + "</td><td width=40>" + p.getLevel() + "</td></tr>");
		}
	...
	private void editCharacter(Player activeChar)
	{
		...
		replyMSG.append("<tr><td width=40>MP:</td><td width=70>" + player.getCurrentMp() + "</td><td width=70>" + player.getMaxMp() + "</td><td width=70>Pvp Kills: " + player.getPvpKills()
		        + "</td></tr>");
		replyMSG.append("<tr><td width=40>Load:</td><td width=70>" + player.getCurrentLoad() + "</td><td width=70>" + player.getMaxLoad() + "</td><td width=70>Pvp Flag: " + player.getPvpFlag()
		        + "</td></tr>");
		replyMSG.append("</table>");
	...
	private void findCharacter(Player activeChar, String CharacterToFind)
	{
		...
		for (Player element : GameObjectsStorage.getAllPlayersForIterate())
			if (element.getName().startsWith(CharacterToFind))
			{
				...
				replyMSG.append("<table width=270>");
				replyMSG.append("<tr><td width=80>Name</td><td width=110>Class</td><td width=40>Level</td></tr>");
				replyMSG.append("<tr><td width=80><a action=\"bypass -h admin_character_list " + element.getName() + "\">" + element.getName() + "</a></td><td width=110>"
				        + HtmlUtils.htmlClassName(element.getClassId().getId()) + "</td><td width=40>" + element.getLevel() + "</td></tr>");
				replyMSG.append("</table>");
			}
	...
	private void setSubclass(final Player activeChar, final Player player)
	{
		...
		{
			activeChar.sendMessage(new CustomMessage("org.mmocore.gameserver.model.instances.L2VillageMasterInstance.NoSubAtThisTime", activeChar));
			return;
		}
