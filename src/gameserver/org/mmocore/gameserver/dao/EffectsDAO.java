...
public class EffectsDAO
{
	...
	public void restoreEffects(Playable playable)
	{
		...
		try
		{
			...
			statement = con
			        .prepareStatement("SELECT `skill_id`,`skill_level`,`effect_count`,`effect_cur_time`,`duration` FROM `character_effects_save` WHERE `object_id`=? AND `id`=? ORDER BY `order` ASC");
	...
	public void insert(Playable playable)
	{
		...
		try
		{
			...
			for (Effect effect : effects)
				if (effect != null && effect.isInUse() && !effect.getSkill().isToggle() && effect.getEffectType() != EffectType.HealOverTime
