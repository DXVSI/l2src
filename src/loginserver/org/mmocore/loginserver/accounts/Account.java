package org.mmocore.loginserver.accounts;
...
import org.mmocore.loginserver.database.L2DatabaseFactory;
...
public class Account
{
	...
	public void update()
	{
		...
		try
		{
			con = L2DatabaseFactory.getInstance().getConnection();
			statement = con
			        .prepareStatement("UPDATE accounts SET password = ?, access_level = ?, ban_expire = ?, allow_ip = ?, bonus = ?, bonus_expire = ?, last_server = ?, last_ip = ?, last_access = ?, points = ? WHERE login = ?");
